...
> * A **thread** is a low level construct and roughly represents actual OS threads.
> * A **task** is a .NET abstraction that basically represents _a promise_ of a separate work. Generally that runs on a separate thread (mostly a managed thread-pool one), managed by the .NET framework.
