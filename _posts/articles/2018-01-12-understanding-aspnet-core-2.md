---
layout: post
title: "Understanding ASP.NET Core 2.0 apps"
excerpt: "Understanding ASP.NET Core 2.0 apps and porting existing MVC apps to Core 2.0"
date: 2018-01-12
tags: [tech, mvc, dotnet, csharp, aspnet]
categories: articles
share: true
...
#### [1] Web Application with `ASP.NET Core Web Application`
...
* Model binding, selection of View
* Layout & ViewStart
...
* The [`Startup`](https://docs.microsoft.com/en-us/aspnet/core/fundamentals/startup) class. It is called from `Main` and has the initial configuration code for the application (which used be mostly in `Application_Start()` method of `Global.asax`) are in `Startup` class. The main methods are
  ...
  * Configure() - required, configure the `http pipeline` by adding necessary [MiddleWare](https://docs.microsoft.com/en-us/aspnet/core/fundamentals/middleware?tabs=aspnetcore2x)
...
* Base namespace changed from `System.Web.Mvc` to `Microsoft.AspNetCore.Mvc`
* For all stuffs `MVC` like `[HttpPost]`, `[FromBody]` etc. use `using Microsoft.AspNetCore.Mvc;`
* There is no `ApiController`, `[FromUri]`	
